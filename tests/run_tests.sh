...
# equality is a 64-bit property. Stage [8/9] checks the float build with a
...
echo "== [1/9] unit tests: flat vs BVH hit equivalence, invariants, refit =="
nvcc tests/test_bvh.cu -o build/test_bvh $NVCC_FLAGS
./build/test_bvh

echo "== [2/9] end-to-end: byte-identical render, flat vs BVH, fixed seed =="
...
echo "== [3/9] Cornell scene (quads/triangle/box/transforms): byte-identical render, flat vs BVH =="
...
echo "== [4/9] badge scene (STL mesh, nested BVH + transforms): byte-identical render, flat vs BVH =="
...
echo "== [5/9] smoke scene (constant media): sanity render, no byte-compare =="
...
echo "== [6/9] tinted-glass scene (Beer-Lambert absorbing dielectric): byte-identical render, flat vs BVH =="
...
echo "== [7/9] viewer headless render (needs SDL2/GLEW/GL dev libs to build; skipped if absent) =="
...
echo "== [8/9] float build (RT_PRECISION=32): flat vs BVH within edge-graze tolerance =="
...
nvcc src/main.cu -o build/rt_flat_f32 -std=c++14 -arch=$ARCH -Isrc -DRT_PRECISION=32 -DRT_SCENE=1 -DUSE_BVH=0 -DRT_SEED=42 -DRT_IMAGE_WIDTH=200 -DRT_SAMPLES=16
nvcc src/main.cu -o build/rt_bvh_f32  -std=c++14 -arch=$ARCH -Isrc -DRT_PRECISION=32 -DRT_SCENE=1 -DUSE_BVH=1 -DRT_SEED=42 -DRT_IMAGE_WIDTH=200 -DRT_SAMPLES=16
...
echo "== [9/9] scene ids: stamping, outermost-wrapper semantics, mutate->refit->re-pick =="
