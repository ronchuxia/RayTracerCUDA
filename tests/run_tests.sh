...
# equality is a 64-bit property. Stage [8/8] checks the float build with a
...
echo "== [1/8] unit tests: flat vs BVH hit equivalence, invariants, refit =="
nvcc tests/test_bvh.cu -o build/test_bvh $NVCC_FLAGS
./build/test_bvh

echo "== [2/8] end-to-end: byte-identical render, flat vs BVH, fixed seed =="
...
echo "== [3/8] Cornell scene (quads/triangle/box/transforms): byte-identical render, flat vs BVH =="
...
echo "== [4/8] badge scene (STL mesh, nested BVH + transforms): byte-identical render, flat vs BVH =="
...
echo "== [5/8] smoke scene (constant media): sanity render, no byte-compare =="
...
echo "== [6/8] tinted-glass scene (Beer-Lambert absorbing dielectric): byte-identical render, flat vs BVH =="
...
echo "== [7/8] viewer headless render (needs SDL2/GLEW/GL dev libs to build; skipped if absent) =="
...
echo "== [8/8] float build (RT_PRECISION=32): flat vs BVH within edge-graze tolerance =="
