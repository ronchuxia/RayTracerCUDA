...
# equality is a 64-bit property. Stage [8/13] checks the float build with a
...
# and the [11/13] guard stage below.
...
echo "== [1/13] unit tests: flat vs BVH hit equivalence, invariants, refit =="
nvcc tests/test_bvh.cu -o build/test_bvh $NVCC_FLAGS
./build/test_bvh

echo "== [2/13] end-to-end: byte-identical render, flat vs BVH, fixed seed =="
...
echo "== [3/13] Cornell scene (quads/triangle/box/transforms): byte-identical render, flat vs BVH =="
...
echo "== [4/13] badge scene (STL mesh, nested BVH + transforms): byte-identical render, flat vs BVH =="
...
echo "== [5/13] smoke scene (constant media): sanity render, no byte-compare =="
...
echo "== [6/13] tinted-glass scene (Beer-Lambert absorbing dielectric): byte-identical render, flat vs BVH =="
...
echo "== [7/13] viewer headless render (needs SDL2/GLEW/GL dev libs to build; skipped if absent) =="
...
echo "== [8/13] float build (RT_PRECISION=32): flat vs BVH within edge-graze tolerance =="
...
echo "== [9/13] scene ids: stamping, outermost-wrapper semantics, mutate->refit->re-pick =="
...
echo "== [10/13] TRS transform node: ray world<->object, Euler rotation, inverse-transpose normals =="
...
echo "== [11/13] regression guard: whole-program (-rdc=false) miscompiles recursive dispatch =="
...
echo "== [12/13] physics module (src/physics.h): collision impulse + drop-settle =="
...
echo "== [13/13] scene physics: authored bodies vs. the geometry they were built from =="
