...
echo "== [1/5] unit tests: flat vs BVH hit equivalence, invariants, refit =="
nvcc tests/test_bvh.cu -o build/test_bvh $NVCC_FLAGS
./build/test_bvh

echo "== [2/5] end-to-end: byte-identical render, flat vs BVH, fixed seed =="
...
echo "== [3/5] Cornell scene (quads/triangle/box/transforms): byte-identical render, flat vs BVH =="
...
echo "== [4/5] badge scene (STL mesh, nested BVH + transforms): byte-identical render, flat vs BVH =="
...
echo "== [5/5] smoke scene (constant media): sanity render, no byte-compare =="
