...
inline void cornell_box() {
    ...
    add_quad(w, point3(343,554,332), vec3(-130,0,0), vec3(0,0,-105), light, allocs);
    add_quad(w, point3(0,0,0),       vec3(555,0,0),  vec3(0,0,555),  white, allocs);
    add_quad(w, point3(555,555,555), vec3(-555,0,0), vec3(0,0,-555), white, allocs);
    ...
    w->add(make_instance(box1, vec3(265,0,295), vec3(0,15,0), vec3(1,1,1)));
    mesh* box2 = new_box(point3(0,0,0), point3(330,330,330), white, allocs, mesh_dtors);
    w->add(make_instance(box2, vec3(130,0,65), vec3(0,-18,0), vec3(0.5,0.5,0.5)));

    // a triangle on the back wall
    add_triangle(w, point3(160,340,554), point3(395,340,554), point3(277.5,470,554), vec3(0,0,-1), blue, allocs);
