...
struct primary_hits {
    ...
    void release() {
        cudaFree(p); cudaFree(id); cudaFree(normal); cudaFree(diffuse); cudaFree(f0); cudaFree(roughness); cudaFree(depth); cudaFree(spec_dist);
        p = nullptr; id = nullptr; normal = nullptr; diffuse = nullptr; f0 = nullptr; roughness = nullptr; depth = nullptr; spec_dist = nullptr; pixels = 0;
    }
};
...
__device__ inline camera::first_hit hit_through_pixel(const camera& cam, int i, int j, const hittable& world, curandState* state) {
    ...
    if (world.hit(r, interval(real(0.001), infinity), rec, state))
        return { color(0,0,0), rec.normal, rec.t, rec.id, rec.p, rec.mat->diffuse_albedo(rec), rec.mat->specular_f0(), rec.mat->roughness(), 0 };
    return { color(0,0,0), vec3(0,0,0), infinity, -1, r.origin() + unit_vector(r.direction()) * real(1e4), color(0,0,0), color(0,0,0), 1, 0 };
}
